...
def upgma_draw_tree(upgmatreepath: str, upgmanodespath: str):
    
    # Read the Newick tree from the file
    with open(upgmatreepath, 'r') as file:
        next(file)  # Skip the first line
        newick_str = file.read()

    # Parse the Newick tree
    parsed_tree = loads(newick_str)[0]

    # Generate the ASCII representation of the tree
    ascii_tree = parsed_tree.ascii_art()

    # Write the ASCII tree to the output file
    with open(upgmanodespath, 'w') as file:
        file.write("The upgma tree generated is shown below: \n\n")
